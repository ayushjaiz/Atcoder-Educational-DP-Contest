...
int fun(int idx, int value_left, vector<vi> &dp)
{
    if (value_left == 0)
        return 0;
    ...
    int notake = 0 + fun(idx - 1, value_left, dp);

    int take = 1e15;
    if (value[idx] <= value_left)
        take = weight[idx] + fun(idx - 1, value_left - value[idx], dp);

    return dp[idx][value_left] = min(take, notake);
}
...
void solve()
{
    ...
    int max_value = 1e5;
    vector<vi> dp(n, vi(max_value + 5, -1));
